...
FEATURE_MIN=20          # paper: track-count based VO-fail
POS_JUMP_MAX=1.5        # m/step, large jump = fail
ROT_JUMP_MAX=0.8        # rad/step, large jump = fail
...
log=open("switching_log_v9_verbose.csv","w",newline="")
w=csv.writer(log)
w.writerow(["time","mode","fused_x","fused_y","fused_z","fused_qx","fused_qy","fused_qz","fused_qw","gt_x","gt_y","gt_z","feat_count","aft_x","aft_y","aft_z"])
cnt=[0]
...
def zero_degenerate(H3, lb, thr):
    # Lambda_p: eigen-decompose, zero out degenerate directions, rebuild safe H
...
def sync_cb(h_msg,v_msg,l_msg,vo_msg):
    ...
    # S_vo: feature-count + jump-based failure (paper Sec III-D proxy)
    ...
    if vo_fail:
        ...
        # Lambda_p: zero degenerate directions in LiDAR-only fallback (Eq16 case2)
        Htt_l=H_l[3:6,3:6]
        Htt_safe=zero_degenerate(Htt_l, lb, thr)
        dp=np.linalg.pinv(Htt_safe, rcond=1e-3) @ (Htt_l@d_l_p)
        if np.linalg.norm(dp) > 2.0:  # sanity clamp, safety net
            dp=d_l_p
        drv=d_l_rv
    else:
        ...
        if status=="Normal":
            ...
            mode="TRANSITION"
            # Eq13: interpolate LiDAR<->VO using sqrt(3*lambda1)
    ...
    w.writerow([t,mode,T_p[0],T_p[1],T_p[2],T_q[0],T_q[1],T_q[2],T_q[3],
                latest_gt["x"],latest_gt["y"],latest_gt["z"],latest_feat_count[0],
                latest_aft["x"],latest_aft["y"],latest_aft["z"]])
    log.flush()

    if fused_pub[0] is not None:
        om=Odometry()
        om.header.stamp=l_msg.header.stamp
        om.header.frame_id="camera_init"
        om.pose.pose.position.x=T_p[0]; om.pose.pose.position.y=T_p[1]; om.pose.pose.position.z=T_p[2]
        om.pose.pose.orientation.x=T_q[0]; om.pose.pose.orientation.y=T_q[1]
        om.pose.pose.orientation.z=T_q[2]; om.pose.pose.orientation.w=T_q[3]
    ...
    cnt[0]+=1
    if cnt[0]%20==0:
        # Real aft_mapped value (final, LOAM-refined output) -- note this
        # is asynchronous, may reflect a slightly earlier frame than T,
        # since A-LOAM's mapping runs at its own rate, not locked to sync_cb.
        aft_str = "(%.2f,%.2f,%.2f)"%(latest_aft["x"],latest_aft["y"],latest_aft["z"]) if latest_aft["x"] is not None else "(pending)"
        print("[%d] %s feat=%d  T=(%.2f,%.2f,%.2f)  FINAL(aft_mapped)=%s"%(
              cnt[0],mode,latest_feat_count[0],T_p[0],T_p[1],T_p[2],aft_str))

        # Real, live equation printout when TIGHTLY_COUPLED actually fires --
        # shows the real Eq.15 weighting in numbers, not just the result.
        if mode=="TIGHTLY_COUPLED":
            tr_Hl = np.trace(H_l6); tr_Hv = np.trace(Hv)
            print("    Eq.15: delta6=(H_l+Hv)^-1 (H_l.d_l + Hv.d_v)")
            print("    trace(H_l)=%.3e  trace(Hv)=%.3e  (Hv/Hl ratio=%.1fx)"%(
                  tr_Hl, tr_Hv, tr_Hv/(tr_Hl+1e-12)))
            print("    d_l_p=(%.3f,%.3f,%.3f)  d_v_p=(%.3f,%.3f,%.3f)  -> dp=(%.3f,%.3f,%.3f)"%(
                  d_l_p[0],d_l_p[1],d_l_p[2], d_v_p[0],d_v_p[1],d_v_p[2], dp[0],dp[1],dp[2]))

def main():
    rospy.init_node("switching_node_v9_verbose",anonymous=True)
    ...
    print("Switching node v6 (feature VO-fail, joint 6DOF, Lambda_p) listening...")
    rospy.spin()

if __name__=="__main__":
    try: main()
    finally:
        log.close(); print("Saved switching_log_v7.csv")
