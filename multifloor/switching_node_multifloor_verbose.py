...
log=open("switching_log_multifloor_verbose.csv","w",newline="")
w=csv.writer(log)
...
def sync_cb(h_msg,v_msg,l_msg,vo_msg):
    ...
    if vo_fail:
        ...
        if status=="Normal":
            ...
            H_sum6=H_l6+Hv6+EPS*np.eye(6)
            try:
                delta6=np.linalg.solve(H_sum6, H_l6@d_l6+Hv6@d_v6)
    ...
    if cnt[0]%20==0:
        ...
        print("    Raw LiDAR Hessian eigenvalues:", " ".join(["%.6e"%x for x in eig]))
        print("    Smallest 3 normalized eigenvalues:", " ".join(["%.6e"%x for x in lb]))
        print("    Degenerate:", degen)
        if mode=="TIGHTLY_COUPLED" and H_l6 is not None:
            tr_Hl=np.trace(H_l6); tr_Hv=np.trace(Hv6)
            print("    Eq.15: delta6=(H_l+Hv)^-1 (H_l.d_l + Hv.d_v)")
            print("    trace(H_l)=%.6e  trace(Hv)=%.6e  (Hv/Hl ratio=%.1fx)"%(tr_Hl,tr_Hv,tr_Hv/(tr_Hl+1e-12)))
            print("    d_l_p=(%.3f,%.3f,%.3f)  d_v_p=(%.3f,%.3f,%.3f)  -> dp=(%.3f,%.3f,%.3f)"%(
                  d_l_p[0],d_l_p[1],d_l_p[2], d_v_p[0],d_v_p[1],d_v_p[2], dp[0],dp[1],dp[2]))

def main():
    rospy.init_node("switching_node_multifloor_verbose",anonymous=True)
    ...
    print("Switching node (multifloor, verbose, real-world) listening...")
    rospy.spin()

if __name__=="__main__":
    try: main()
    finally:
        log.close(); print("Saved switching_log_multifloor_verbose.csv")
