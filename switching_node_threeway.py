...
log=open("switching_log_threeway_rawlog.csv","w",newline="")
w=csv.writer(log)
w.writerow(["time","mode","fused_x","fused_y","fused_z","fused_qx","fused_qy","fused_qz","fused_qw",
            "gt_x","gt_y","gt_z","feat_count","aft_x","aft_y","aft_z",
...
def sync_cb(h_msg,v_msg,l_msg,vo_msg):
    ...
    if vo_fail:
        ...
        if status=="Normal":
            ...
            conf = (chanR_r*1.0 + chanR_t*1.0 + degen_T*2.0) / 4.0
...
def main():
    rospy.init_node("switching_node_threeway",anonymous=True)
    ...
    print("Switching node (threeway) listening...")
    rospy.spin()

if __name__=="__main__":
    try: main()
    finally:
        log.close(); print("Saved switching_log_threeway_rawlog.csv")
